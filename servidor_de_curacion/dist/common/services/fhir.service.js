...
const jsum_1 = __importDefault(require("jsum"));
const tslog_1 = require("tslog");
const log = new tslog_1.Logger();
class FhirService {
    constructor(isSource, fhirURL) {
        if (fhirURL) {
            if (isSource) {
                environment_1.environment.server.config.source.baseUrl = fhirURL;
            }
            else {
                environment_1.environment.server.config.target.baseUrl = fhirURL;
            }
        }
        this.config = isSource ? environment_1.environment.server.config.source : environment_1.environment.server.config.target;
    ...
    postBatch(resources, method) {
        ...
        for (const resource of resources) {
            ...
            (_a = transactionResource.entry) === null || _a === void 0 ? void 0 : _a.push({
                resource,
                request
            });
        }
        return axios_1.default.post(this.config.baseUrl, transactionResource, { headers: this.config.headers, httpAgent });
    }
    /**
    ...
    validate(resources) {
        ...
        for (const resource of resources) {
            let url;
            if (((_b = (_a = resource.meta) === null || _a === void 0 ? void 0 : _a.profile) === null || _b === void 0 ? void 0 : _b.length) && resource.meta.profile[0]) {
            ...
            (_c = transactionResource.entry) === null || _c === void 0 ? void 0 : _c.push({
                resource,
                request
            });
        }
    ...
    validateResource(resource) {
        ...
        return axios_1.default.post(url, resource, { headers: this.config.headers, httpAgent });
    }
    // HELPERS
    getResourceBy(resourceType, query, value) {
        const url = this.config.baseUrl + '/' + resourceType + '?' + query + '=' + value;
        return axios_1.default.get(url);
    }
    add_element(array, resource, attribute) {
        const should_add_element = new Array(array.length).fill(true);
        array.forEach((value, index) => {
            resource[attribute].forEach((iter_item, resource_index) => {
                if ((iter_item === null || iter_item === void 0 ? void 0 : iter_item.uid) === value.uid) {
    ...
    prepareResources(resources) {
        ...
        if (!environment_1.environment.additionalRules.postOrganization) {
            ...
            resources.forEach(resource => {
                if (resource.resourceType === 'Organization') {
                    ...
                    unique_resources[jsum_1.default.digest(resource, 'SHA256', 'hex')] = resource;
                }
            });
            for (const [_, organization] of Object.entries(organizations)) {
                const resource = organization;
                if (resource.resourceType === 'Organization') {
                    organizationPromises.push(this.getResourceBy(resource.resourceType, 'name', resource.name).then(responseResource => {
            ...
            resources.forEach(resource => { unique_resources[jsum_1.default.digest(resource, 'SHA256', 'hex')] = resource; });
        }
            ...
            for (const [checksum_key, resource_value] of Object.entries(unique_resources)) {
                ...
                if (resource.resourceType === 'ImagingStudy') {
                    ...
                    if (cached_resource) {
                        ...
                        promises.push(this.getResourceBy(resource.resourceType, '_id', resource.id).then(responseResource => {
                            // Fix reference to Organization
                            if (!environment_1.environment.additionalRules.postOrganization) {
                                unique_resources[checksum_key] = this.fixOrganization(unique_resources[checksum_key], organizations);
                            }
    ...
    cleanInternalCache() {
        if (this.irc_queue.length > environment_1.environment.cacheOptions.maxResources) {
            for (let i = 0; i < environment_1.environment.cacheOptions.onMaxDeleteCount; i++) {
